...
public class ScrollPoints extends LinearLayout {
	...
	public void initPoints(Context context, int count, int selected) {
		...
		for (int i = 0; i < count; i++) {
			...
			if (i == selected) {
				slidePot.setImageResource(R.mipmap.guide_indicate_highlight);
			} else {
				slidePot.setImageResource(R.mipmap.guide_indicate);
			}
	...
	public void changeSelectedPoint(int position) {
		for (int i = 0; i < mPoints.size(); i++) {
			...
			if (i == position) {
				point.setImageResource(R.mipmap.guide_indicate_highlight);
			} else {
				point.setImageResource(R.mipmap.guide_indicate);
			}
		}
	}
